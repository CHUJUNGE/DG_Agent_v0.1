...
|---|---|---|
| 模块1：我的近况和生活重心 | 理解受访者是谁、生活重心、兴趣圈层、时间精力与消费状态 | 对应 RQ1；本项目依赖代际与人群变化，需保留生活背景，但不直接进入笔 |
| 模块2：我的一天和常出现的地方 | 捕捉学习 / 工作 / 通勤 / 社交 / 兴趣活动中的真实触点 | 对应 RQ1、RQ2；用日常场景承接后续书写场景，而非泛画像 |
| 模块3：我的文具和随身物件 | 建立当前笔类与相邻物件系统，包括拥有、分类、收纳、携带、闲置 | 对应 RQ3；笔类为物件驱动品类，需先收集 object system |
| 模块4：我是怎么写、记、画和表达的 | 理解书写行为、数字工具替代、纸笔不可替代场景 | 对应 RQ2、RQ3；围绕书写价值变化展开 |
| 模块5：我这几天真实用到笔的时候 | 用短期日记记录真实发生的书写 / 用笔 / 浏览 / 购买触点 | 对应 RQ2、RQ3、RQ4；避免只靠回忆，记录事实与即时评价 |
| 模块6：我是怎么选笔、买笔和留下笔的 | 理解购买、尝新、复购、被种草、价格、渠道、闲置与替换逻辑 | 对应 RQ3、RQ4；连接产品机会与商业转化 |
| 模块7：我的好笔标准 | 拆解好用、好看、有质感、好玩、适合送礼 / 分享等具体标准 | 对应 RQ4；产品创新前必须先建立可转化的评价标准 |
| 模块8：我的笔像什么 | 理解笔在生活中的角色、人格化、情绪价值、自我表达与变化 | 对应 RQ3、RQ4、RQ5；回应“非刚需但规模不小”的价值重构 |
| 模块9：我的下一支笔 | 在已有事实基础上收敛未来产品机会、跨人群放大方向与概念原型线索 | 对应 RQ5；未来想象需有边界，避免空泛新品脑暴 |
...
- 不能改动 / 删除：  
  ...
  2. 模块3 的“笔的全家福 / 常用 / 喜欢 / 闲置 / 分类 / 收纳 / 携带”不能删，只能合并或改写。  
  3. 模块5 的真实记录链路不能删，尤其是场景、触发、替代方式、选择理由、体验评价。  
  4. 模块7 的具体好笔标准不能直接压缩成泛问“看重什么”，需保留具体例子。  
  5. 模块9 的未来想象必须绑定真实场景和真实问题，不能改成完全开放的新品脑暴。
...
- 可能过重或过硬的题面：  
  模块5 每日记录题较多，需 wording agent 降低填写压力，可改成“选今天最有代表性的 1-3 次”；模块7 标准拆解较细，需避免变成 checklist；模块9 的未来产品想象需更自然、更像聊天任务。
